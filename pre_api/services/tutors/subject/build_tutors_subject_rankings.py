...
def build_tutors_subject_rankings(subject_id: int, kind: str = "best-performance", limit: int = 10):
    ...
    try:
        ...
        data = analyzer.tutors_rankings_analysis(subject_id, scope='subject', version=version, connector=connector, kind=kind, limit=limit)
        return data
    finally:
        try:
            connector.close()
