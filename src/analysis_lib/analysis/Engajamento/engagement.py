...
class Engagement(Indicator):
    ...
    def student_analysis(self, subject_id, student_id, version, connector):
        ...
        posts_por_usuario = (
            df_posts.groupby('user_id')['post_id_required']
            .count().reset_index()
            .rename(columns={'post_id_required': 'num_posts_required'})
        )
        ...
        df_final["posts_required_label"] = df_final["num_posts_required"].apply(discretize)

        df_final["user_id"] = pd.to_numeric(df_final["user_id"], errors="coerce")
        sid = pd.to_numeric(student_id, errors="coerce")

        student_df = df_final.loc[df_final["user_id"] == sid]

        if student_df.empty:
            return None  

        row = student_df.iloc[0].where(pd.notna(student_df.iloc[0]), None).to_dict()
        return row
